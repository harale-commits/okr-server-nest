...
describe('KeyresultsCompletionService', () => {
    ...
    beforeEach(async () => {
      ...
      service = module.get<KeyresultsCompletionService>(KeyresultsCompletionService)
    })
    ...
    describe('isCompleted', () => {
      it('should return true if current value and target value are same', () => {
        expect(service.isCompleted({...dummyKeyResult, currentValue : 1, targetValue : 1})).toBeTruthy()
      })

      it('should return false if current value and target value are not same', () => {
        expect(service.isCompleted({...dummyKeyResult, currentValue : 1, targetValue : 3})).toBeFalsy()

      })

      it("should return true if the current value is greater than target value", ()=>{
        expect(service.isCompleted({...dummyKeyResult, currentValue : 5, targetValue : 3})).toBeTruthy()
      })
